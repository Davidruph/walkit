...
function getDistance($addressFrom, $addressTo, $mode, $unit = 'K')
{
    ...
    $geocodeFrom = file_get_contents('https://maps.googleapis.com/maps/api/geocode/json?address=' . $formattedAddrFrom . '&sensor=false&mode=' . $mode . '&key=' . $apiKey);
    ...
    $geocodeTo = file_get_contents('https://maps.googleapis.com/maps/api/geocode/json?address=' . $formattedAddrTo . '&sensor=false&mode=' . $mode . '&key=' . $apiKey);
